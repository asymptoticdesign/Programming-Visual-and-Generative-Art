//  Title: Rectangles (Soln 6)
//  Description: Solution to Modification 6 to Rectangles
...
void setup(){
  ...
  for(int i=0;i < number_of_squares;i++){
    fill(0,0,255*abs(sin(i*PI / number_of_squares)));
...
void draw(){
  for(int i=0;i < number_of_squares;i++){
    fill(0,0,255*abs(sin((i+count)*PI / number_of_squares)));
    rect(i*square_size,0,square_size,height);
  }
  delay(100);
  count++;
